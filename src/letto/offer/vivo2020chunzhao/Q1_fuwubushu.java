...
public class Q1_fuwubushu {
    ...
    private static int solution(int totalDisk, int totalMemory, List<Service> services) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = totalDisk; j>=services.get(i).getDisk(); j--){
                for (int k = totalMemory; k>=services.get(i).getMemory();k--){
                    dp[j][k] = Math.max(dp[j-services.get(i).getDisk()][k-services.get(i).getMemory()]+services.get(i).getusers(),
                            dp[j][k]);
                }
            }
        }
